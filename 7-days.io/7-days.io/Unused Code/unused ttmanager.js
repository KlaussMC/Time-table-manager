'use strict';
var ttmanager = () => document.querySelector('#editorField').style.display = 'block';
  ...
  for (day = 1; day <= days; day++) {
    for (period = 1; period < periods; period++) {
      if (period > 1) {
        ...
        var c = (getPeriodTimes(day, period)) ? function () {
          console.log(getPeriodTimes(day, period));
          let newFinishTime;
          newFinishTime = addTimes(getPeriodTimes(day, period).to, subTimes(getPeriodTimes(day, period).to, getPeriodTimes(day, period).from))
          var d = (newFinishTime) ? function () { setPeriodTimes(day, period, getPeriodTimes(day, period).to, newFinishTime) } : () => { return null; }
        } : () => { return null }
...
var changedays = val => { days = val; updateTable() }
var changeperiods = val => { periods = val; updateTable() }
